...
require_once('node_modules/@psu-online-education/smart-datetime/SmartDatetime.php');
...
function addCustomExtension(Environment $env) {
  ...
  $env->addFunction(new TwigFunction('get_component_stylesheets', function () {
      $manifests = [];
      foreach (glob('node_modules/@psu-online-education/*/package.json') as $manifest) {
      ...
      foreach (array_keys($manifests) as $manifest) {
        ...
        $potential_css_file = "node_modules/@psu-online-education/$component/dist/styles.css";
    ...
    $env->addFunction(new TwigFunction('get_component_scripts', function () {
        $scripts = '';
        foreach (glob('node_modules/@psu-online-education/*/dist/scripts.js') as $component) {
    ...
    $env->addFunction(new TwigFunction('get_sprites', function() {
        return file_get_contents('node_modules/@psu-online-education/sprite/dist/sprites.svg');
    }));
}
